...
const joinEvent = async (event) => {
	...
	try {
		const response = await fetch("/api/bedrijf/join-event", {
			method: "POST",
			headers: {
				"Content-Type": "application/json",
				"bedrijf-email": localStorage.getItem("bedrijfEmail") || ""
			},
		...
		if (response.ok && data.success) {
			...
			setTimeout(() => {
				router.push({ name: "bedrijf-profiel" });
			}, 1200);
		} else {
...
onMounted(async () => {
	// Checken of bedrijf in evenement zit
		...
		try {
			...
			if (response.ok) {
				...
				if (profile.joinedEventId || profile.hasJoinedEvent) {
					// Als bedrijf in evenement zit, versturen naar home-pagina
					router.push({ name: "bedrijf-profiel" });
					return;
				}
			}
	...
	<div class="page-wrapper">
		<div class="phone-frame student-profile-page">
			<header class="top-bar">
				<span class="top-bar-title">Handshake Bedrijf</span>
