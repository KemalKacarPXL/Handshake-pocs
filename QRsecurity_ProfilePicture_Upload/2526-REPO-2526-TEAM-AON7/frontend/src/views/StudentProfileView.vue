...
const picErrorMessage = ref("");
const picFileInput = ref(null);
const isDeletingPic = ref(false);
const profile = ref({
  firstname: "",
  lastname: "",
  email: "",
...
const deletePic = async () => {
  if (!confirm("Weet je zeker dat je je profielfoto wilt verwijderen? Je QR-code zal hierdoor tijdelijk niet meer werken.")) return;
  
  isDeletingPic.value = true;
  picMessage.value = "";
  picErrorMessage.value = "";

  try {
    const response = await fetch(`/api/student/profile/picture?email=${encodeURIComponent(currentEmail.value)}`, {
      method: "DELETE",
    });

    if (!response.ok) throw new Error("Verwijderen mislukt");

    picMessage.value = "Profielfoto verwijderd.";
    await loadProfile(); // Herlaad om de wijziging (en de status) door te voeren
  } catch {
    picErrorMessage.value = "Kan foto niet verwijderen.";
  } finally {
    isDeletingPic.value = false;
  }
};
...
const mapStudentData = (data, fallback) => ({
  ...
  hasProfilePicture: Boolean(data.hasProfilePicture),
});

const onPicSelected = async (event) => {
  ...
  try {
    ...
    picMessage.value = "Profielfoto succesvol geupload!";
    await loadProfile(); // Herlaad profiel om foto direct te tonen
  } catch {
    picErrorMessage.value = "Profielfoto uploaden mislukt.";
  } finally {
    isUploadingPic.value = false;
    if (picFileInput.value) picFileInput.value.value = "";
  }
};
  ...
  <div class="page-wrapper">
    <div class="phone-frame student-profile-page">
      ...
      <main class="profile-content">
        ...
        <section class="card cv-card">
          <h3>Profielfoto</h3>
          <div v-if="profile.hasProfilePicture" class="cv-file" style="justify-content: center; background: transparent;">
             <img :src="`/api/student/profile/picture?email=${encodeURIComponent(currentEmail)}`" alt="Profielfoto" style="width: 100px; height: 100px; border-radius: 50%; object-fit: cover; border: 2px solid #0e66c2;" />
          </div>
          <p v-else class="cv-empty">Er is nog geen profielfoto geupload. (Verplicht voor QR-code)</p>
          ...
          <div class="cv-actions">
            ...
            <button
              ...
              @click="picFileInput.click()"
            >
              {{ isUploadingPic ? "Uploaden..." : (profile.hasProfilePicture ? "Foto wijzigen" : "Foto uploaden") }}
            </button>
            <button
              type="button"
              class="btn-secondary"
              v-if="profile.hasProfilePicture"
              :disabled="isDeletingPic"
              @click="deletePic"
            >
              {{ isDeletingPic ? "Verwijderen..." : "Foto verwijderen" }}
            </button>
          </div>
          <p v-if="picMessage" class="cv-feedback success">{{ picMessage }}</p>
          <p v-if="picErrorMessage" class="cv-feedback error">{{ picErrorMessage }}</p>
        </section>
